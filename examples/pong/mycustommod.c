...
static grub_err_t
grub_cmd_draw_graphics (grub_command_t cmd __attribute__ ((unused)),
                        ...
                        char **args __attribute__ ((unused)))
{
    ...
    while (1) {
        ...
        // Draw score display using simple graphics
        // Draw score boxes in top corners
        int score_box_size = 20;
        int score_box_x1 = 50;
        int score_box_x2 = info.width - 70;
        int score_box_y = 20;
        
        // Draw score boxes (simple rectangles to represent scores)
        for (int i = 0; i < score1; i++) {
            grub_video_fill_rect(ball_color, score_box_x1 + i * 25, score_box_y, score_box_size, score_box_size);
        }
        for (int i = 0; i < score2; i++) {
            grub_video_fill_rect(ball_color, score_box_x2 + i * 25, score_box_y, score_box_size, score_box_size);
        }

        // Draw control indicators (small rectangles to show which keys to press)
        // Left side: W/S indicators
        grub_video_fill_rect(paddle_color, 5, info.height - 40, 8, 8);  // W indicator
        grub_video_fill_rect(paddle_color, 5, info.height - 20, 8, 8);  // S indicator
        
        // Right side: Arrow indicators  
        grub_video_fill_rect(paddle_color, info.width - 15, info.height - 40, 8, 8);  // Up indicator
        grub_video_fill_rect(paddle_color, info.width - 15, info.height - 20, 8, 8);  // Down indicator
        
        // Quit indicator (Q key) - small square in bottom center
        grub_video_fill_rect(ball_color, info.width/2 - 4, info.height - 20, 8, 8);  // Q indicator
...
GRUB_MOD_INIT(mycustommod)
{
    // Register the command
    grub_register_command ("draw_graphics", grub_cmd_draw_graphics,
                          "draw_graphics", "Play Pong game with graphics");
}

GRUB_MOD_FINI(mycustommod)
{
    /* Cleanup if needed */
}
